...
export default function GuidedClient() {
    ...
    if (pid) {
      (async () => {
        try {
          ...
          if (steps.ideas) {
            setIdeas(steps.ideas.options || []);
            setChosenIdeaIdx(steps.ideas.chosen ?? null);
          }
          if (steps.outline) {
            setOutlines(steps.outline.options || []);
            setChosenOutlineIdx(steps.outline.chosen ?? null);
          }
          if (steps.script) {
            setScripts(steps.script.options || []);
            setChosenScriptIdx(steps.script.chosen ?? null);
          }
          if (steps.deck) {
            setDeckJson(steps.deck.deckJson || null);
          }
        } catch {}
      })();
    }
  ...
  async function ensureProject() {
    ...
    router.replace(`/guided?pid=${created.project_id}`);
  ...
  async function onGenIdeas() {
    ...
    try {
      ...
      setIdeas(options.map((o) => ({ ...o })));
      setChosenIdeaIdx(null);
      setStep(0);
      const { uid, pid } = await ensureProject();
      await ProjectsAPI.saveStep(uid, pid, "ideas", {
        options,
        chosen: null,
        meta: { genre, tone, language, seed },
      });
    } catch (e) {
      toastErr(e);
    } finally {
  ...
  async function onGenOutlines() {
    ...
    try {
      ...
      setOutlines(options.map((o) => ({ ...o })));
      setChosenOutlineIdx(null);
      setStep(1);
      const { uid, pid } = await ensureProject();
      await ProjectsAPI.saveStep(uid, pid, "outline", {
        options,
        chosen: null,
        input: idea,
      });
  ...
  async function onGenScripts() {
    ...
    setBusy(true);
    try {
      const { options } = await GenAPI.scripts({
        outline: outline.outline,
        style: "cinematic, grounded, commercial",
        language,
      });
      setScripts(options.map((o) => ({ ...o })));
      setChosenScriptIdx(null);
      setStep(2);
      const { uid, pid } = await ensureProject();
      await ProjectsAPI.saveStep(uid, pid, "script", {
        options,
        chosen: null,
        input: outline,
      });
  ...
  async function onBuildDeck() {
    ...
    if (!idea || !outline || !script)
      return toastErr(new Error("Choose items in all previous steps."));
    setBusy(true);
    try {
    ...
    <div className={styles.container}>
      ...
      {step === 0 && (
        <div className={styles.panel}>
          ...
          <textarea
            ...
            placeholder="Premise / seed"
            value={seed}
            onChange={(e) => setSeed(e.target.value)}
          />
          <div className={styles.footer}>
            <button className={styles.primary} onClick={onGenIdeas} disabled={busy}>
              Generate Ideas
            </button>
          </div>
        </div>
      ...
      {ideas.length > 0 && (
        <>
          ...
          <div className={styles.footer}>
            <button className={styles.secondary} onClick={() => setIdeas([])}>
              Reset ideas
            </button>
            <button
              className={styles.primary}
              onClick={onGenOutlines}
              disabled={busy || chosenIdeaIdx == null}
            >
              Generate Outlines
            </button>
          </div>
      ...
      {outlines.length > 0 && (
        <>
          ...
          <div className={styles.footer}>
            <button className={styles.secondary} onClick={() => setOutlines([])}>
              Reset outlines
            </button>
            <button
              className={styles.primary}
              onClick={onGenScripts}
              disabled={busy || chosenOutlineIdx == null}
            >
      ...
      {scripts.length > 0 && (
        <>
          ...
          <div className={styles.footer}>
            <button className={styles.secondary} onClick={() => setScripts([])}>
              Reset scripts
            </button>
            <button
              className={styles.primary}
              onClick={onBuildDeck}
              disabled={busy || chosenScriptIdx == null}
            >
              Build Deck JSON
            </button>
          </div>
        ...
        <>
          ...
          <div className={styles.panel}>
            <textarea
              ...
              onChange={(e) => {
                try {
                  setDeckJson(JSON.parse(e.target.value));
                } catch {}
              }}
            />
            <div className={styles.footer}>
