...
export default function Guided() {
  return <main className="p-6">Guided – coming soon</main>;
}
...
export default function Guided() {
  ...
  const [tone, setTone] = useState("bold, cinematic");
  ...
  async function stepIdea() {
    setBusy(true);
    const r = await api.post("/gen/idea", { genre, tone, seed, language });
  ...
  async function exportDeck(fmt: "pdf" | "docx") {
    if (!deck) return;
    const r = await api.post("/export", { deck_json: deck, format: fmt });
      ...
      <div className="grid gap-3">
        ...
        <div className="flex gap-2">
          <select value={genre} onChange={e=>setGenre(e.target.value)} className="bg-white/5 rounded px-3 py-2">
            <option>thriller</option><option>drama</option><option>rom-com</option><option>action</option><option>satire</option><option>fantasy</option>
          </select>
        ...
        <textarea rows={6} value={idea} onChange={e=>setIdea(e.target.value)} className="bg-white/5 rounded p-3"></textarea>
        ...
        <textarea rows={10} value={outline} onChange={e=>setOutline(e.target.value)} className="bg-white/5 rounded p-3"></textarea>
        ...
        <textarea rows={10} value={script} onChange={e=>setScript(e.target.value)} className="bg-white/5 rounded p-3"></textarea>
